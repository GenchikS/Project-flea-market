...
import css from "./GardenComponent.module.css";
  ...
  return (
    <div className={css.containerGardenComponent}>
      ...
      <div>
        {items.length > 0 &&
          items.map((item) => (
            <ul key={item.id} className={css.tableContainerGardenComponent}>
      ...
      <div>
        {loadig && (
          <div className={css.containerLoadingData}>
            <p className={css.textLoadingData}>Пошук сад та город ...</p>
            <MyComponent />
          </div>
        )}
      </div>
